...
const SubjectStatsChart: React.FC<{stats: SubjectStats[];title: string;isDarkMode: boolean;onClose: () => void;}> = ({ stats, title, isDarkMode, onClose }) => {
        ...
        tooltip: {
          callbacks: {
            label: (context: any) => {
              ...
              return `المعدل: ${item.mean} | الإنحراف المعياري: ${item.stdDev} | ملاحظة: ${item.remark}`;
            }
          }
        },
      },
