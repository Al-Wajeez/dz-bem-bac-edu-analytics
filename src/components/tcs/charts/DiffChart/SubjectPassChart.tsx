...
const SubjectPassChart: React.FC<SubjectStatsChartProps> = ({ data, title, termA, termB, isDarkMode, onClose }) => {
    ...
    const chartData = {
      ...
      datasets: [
        {
          label: 'المتوسط' + {termA},
          data: data.map(item => item.passPercentageA),
        ...
        {
          label: 'المتوسط' + {termB},
          data: data.map(item => item.passPercentageB),
          backgroundColor: colors,
          borderColor: colors.map(color => color.replace('70%', '50%')),
          borderWidth: 1,
        },
...
export default SubjectPassChart;
