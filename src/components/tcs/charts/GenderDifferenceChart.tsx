...
const GenderDifferenceChart: React.FC<{differences: GenderDifferenceStats[];title: string;isDarkMode: boolean;onClose: () => void;}> = ({ differences, title, isDarkMode, onClose }) => {
    ...
    const chartData = {
      ...
      datasets: [
        {
          label: 'الذكور',
          data: differences.map(item => item.maleMean),
        ...
        {
          label: 'الإناث',
          data: differences.map(item => item.femaleMean),
          backgroundColor: colors,
          borderColor: colors.map(color => color.replace('70%', '50%')),
          borderWidth: 1,
        },
...
export default GenderDifferenceChart;