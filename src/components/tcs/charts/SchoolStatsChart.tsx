...
const SchoolStatsChart: React.FC<{stats: SchoolStats[];title: string;isDarkMode: boolean;onClose: () => void;}> = ({ stats, title, isDarkMode, onClose }) => {
  const chartData = {
    labels: stats.map(item => item.school),
    datasets: [
      {
        label: 'معدل القسم',
