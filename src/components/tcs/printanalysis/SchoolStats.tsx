...
const SchoolPerformancePrintableAnalysis: React.FC<{ stats: SchoolStats[] }> = ({ stats }) => {
  return (
    <div id="schoolStatsAnalysis" className="hidden-printable space-y-8">
      {stats.map((stat) => {
        ...
        return (
          <div key={stat.school}>
            <h3 className="text-lg font-bold mb-4">تحليل الأداء العام ل{stat.school}</h3>

            <div className="whitespace-pre-line leading-8 text-right space-y-4">

              {/* فقرة أولى: عرض المعطيات */}
              <p>
                تُصنّف الوضعية العامة لمؤسسة {stat.school} ضمن فئة {remark} من حيث الأداء، 
...
export default SchoolPerformancePrintableAnalysis;
