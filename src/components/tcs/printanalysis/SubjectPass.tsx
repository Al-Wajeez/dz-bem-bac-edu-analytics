...
const SubjectPassPrintableAnalysis: React.FC<{ passRates: SubjectPassStats[] }> = ({ passRates }) => {
    const getGlobalPassSummary = (): JSX.Element => {
      const excellentSubjects: string[] = [];
      const averageSubjects: string[] = [];
      const weakSubjects: string[] = [];

      passRates.forEach(({ subject, passPercentage }) => {
        if (passPercentage >= 85) {
          excellentSubjects.push(`${subject} (${passPercentage.toFixed(2)}%)`);
        } else if (passPercentage >= 50) {
          averageSubjects.push(`${subject} (${passPercentage.toFixed(2)}%)`);
        } else {
          weakSubjects.push(`${subject} (${passPercentage.toFixed(2)}%)`);
        }
      });
    ...
    return (
      <div id="subjectPassAnalysis" className="hidden-printable">
        <h3 className="text-lg font-bold mb-4">التحليل الشامل لنسب النجاح حسب المواد</h3>
        {getGlobalPassSummary()}
      </div>
    );
  };

export default SubjectPassPrintableAnalysis;