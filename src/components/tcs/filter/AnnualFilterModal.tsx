...
import type { Student } from '../AnnualAnalysis';
...
export const FilterModal: React.FC<Props> = ({ isOpen, onClose, students, isDarkMode }) => {
  ...
  // Handlers for new selects
  const handleSubjectTermChange = useCallback((vals: any) => {
    setFilter('subjectTerm', vals.map((v: any) => v.value));
    setFilter('subject', []); // Reset subject when term changes
  }, [setFilter]);

  //const handleSubjectChange = useCallback((vals: any) => {
  //  setFilter('subject', vals.map((v: any) => v.value));
  //}, [setFilter]);
    ...
    >
      ...
      <div className={`p-6 w-full max-w-4xl ${isDarkMode ? 'bg-gray-800 text-white' : 'bg-white text-gray-900'}`}>
        ...
        <div className="space-y-4">
          <div>
            <label className="block text-ms font-bold mb-2">الفصل</label>
            <Select
              isMulti
              options={subjectTermOptions}
              value={subjectTerm.map((t: string) => ({ value: t, label: t }))}
              onChange={handleSubjectTermChange}
          ...
          <div className="flex flex-wrap grid grid-cols-1 sm:grid-cols-2 gap-4 w-full">
            {/*<div>
              <label className="block text-ms font-bold mb-2">المادة</label>
              <Select
                isMulti
                options={subjectTerm.length === 1 ? subjectsByTerm[subjectTerm[0]] : []}
                value={subject.map((s: string) => ({ value: s, label: s }))}
                onChange={handleSubjectChange}
                placeholder="اختر المادة"
                styles={selectStyles}
                isClearable
                isDisabled={subjectTerm.length !== 1}
              />
            </div>*/}
          </div>
        </div>
